...
public final class ScriptCrypto {
    ...
    /** Frida-compile packages must be loaded by Gadget's native module loader. */
    public static boolean isModuleBundle(String source) {
        if (source == null) return false;
        int offset = source.startsWith("\uFEFF") ? 1 : 0;
    ...
    public static final class Bootstrap {
        ...
        public String render() {
            String[] lines = new String[]{
                    ...
                    "  var logWriter = null;",
                    "  function consoleLog(m) { try { originalLog(m); } catch (e) { } }",
                    "  // Write diagnostics through liblog without requiring a Java bridge.",
                    "  function logcat(m, priority, tag) {",
                    "    try {",
                    "      if (logWriter) {",
                    "        logWriter(priority || 4, Memory.allocUtf8String(tag || CFG.logTag), Memory.allocUtf8String(m));",
                    "        return true;",
                    "      }",
                    "      var addr = null;",
                    "      try {",
                    ...
                    "      logWriter = new NativeFunction(addr, 'int', ['int', 'pointer', 'pointer']);",
                    "      logWriter(priority || 4, Memory.allocUtf8String(tag || CFG.logTag), Memory.allocUtf8String(m));",
                    "      return true;",
                    ...
                    "    if (console.__zfmLogcatBridge) { return; }",
                    ...
                    "      } catch (e) { try { return String(value); } catch (_) { return '[unprintable]'; } }",
                    "    }",
                    "    function forward(args, priority) {",
                    "      if (forwarding) { return; }",
                    "      forwarding = true;",
                    "      try {",
                    "        var values = [];",
                    "        for (var i = 0; i < args.length; i++) { values.push(format(args[i])); }",
                    "        var lines = values.join(' ').split(String.fromCharCode(0)).join('\\\\0').split(/\\r?\\n/);",
                    "        for (var n = 0; n < lines.length; n++) {",
                    "          var line = lines[n], offset = 0;",
                    "          do {",
                    "            var end = Math.min(offset + 900, line.length);",
                    "            var last = line.charCodeAt(end - 1);",
                    "            if (end < line.length && last >= 0xd800 && last <= 0xdbff) { end--; }",
                    "            logcat('[' + CFG.name + '] ' + line.slice(offset, end), priority, 'ZFM-Script');",
                    "            offset = end;",
                    "          } while (offset < line.length);",
                    "        }",
                    "      } catch (e) { } finally { forwarding = false; }",
                    "    }",
                    "    [['log',4], ['info',4], ['warn',5], ['error',6], ['debug',3]].forEach(function (entry) {",
                    "      var original = typeof console[entry[0]] === 'function' ? console[entry[0]] : console.log;",
                    "      console[entry[0]] = function () {",
                    "        try { return original.apply(console, arguments); }",
                    "        finally { forward(arguments, entry[1]); }",
                    "      };",
                    "    });",
                    "    Object.defineProperty(console, '__zfmLogcatBridge', { value: true });",
                    "  }",
                    ...
                    "      try { Script.evaluate('/' + name, src); return true; }",
                    "      catch (e) { log('Script.evaluate failed: ' + e); return false; }",
                    "    }",
                    "    if (typeof Script !== 'undefined' && Script !== null",
                    "        && typeof Script.load === 'function') {",
                    "      try {",
                    "        return Promise.resolve(Script.load('/' + name, src)).then(function () { return true; },",
                    "          function (e) { log('Script.load failed: ' + e); return false; });",
                    "      } catch (e) { log('Script.load failed: ' + e); return false; }",
                    "    }",
                    ...
                    "      log(success ? CFG.msgReady : CFG.msgFail + 'see logcat -s ' + CFG.logTag);",
                    "      log('done: ok=' + success);",
                    "    }",
                    "    if (ok && typeof ok.then === 'function') { ok.then(done); } else { done(ok); }",
                    "  }",
                    "  try { main(); }",
